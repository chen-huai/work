...
def get_list(name,element,wavelenth,row,n):    
    ...
    name_list=os.path.basename(os.path.realpath(path)).split('-')
    now=name_list[0]
    lists=[]
    list=[]
    for line in ph_date:   
        if  ('%s'%name in line[0]) and ('%s'%element in line[2]) and ('%s'%wavelenth in line[2]):            
            lists.append(line) 
    ...
    if n<=9: 
        ...
        if lists!=[]:
            ...
            if n<=3:      
                excel.Sheets("CRM recorvery rate").Cells(row,maxcolumn).Value=float(lists[0][4])*int(float(list[1])*250)/float(list[1])
            elif n==4:
                excel.Sheets("CRM recorvery rate").Cells(row,maxcolumn).Value=float(lists[0][4])*int(float(list[1])*250)*10/float(list[1])   
            elif n<=6:
                excel.Sheets("CRM recorvery rate").Cells(row,maxcolumn).Value=float(lists[0][4])*int(float(list[1])*250)*25/float(list[1]) 
            else:
                excel.Sheets("CRM recorvery rate").Cells(row,maxcolumn).Value=float(lists[0][4])     
        else:
        ...
        for list in lists:
            ...
            excel.Sheets("Data 1").Cells(row,maxcolumn).Value= float(list[4])
            maxcolumn+=1
        if n==16:
            maxcolumn=2
...
get_list('CC','Pb', '220.353',2,10)
get_list('CC','Cd','228.802',3,11)
get_list('CC','Ni','231.604',4,12)
get_list('CC','As','188.980',5,13)
get_list('CC','Hg','184.887',6,14)
get_list('CC','Sb','206.834',7,15)
get_list('CC','Sn','189.925',8,16)
